...
function showlist(booklist) {
  var lista = document.getElementById("caja");
  removeChildren(lista);
  var count = booklist.length > 15 ? 15 : booklist.length;
  for (var i = 0; i < count; i++) {
    lista.appendChild(createBookElement(booklist[i]));
  }
  g_lastcount = count;
  var lista = document.getElementById("cantidad");
  removeChildren(lista);
  lista.appendChild(
    document.createTextNode("Mostrando " + count + " de " + booklist.length)
  );
}
